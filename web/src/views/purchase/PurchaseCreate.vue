...
function submitForm() {
  form2.value?.validate(async (errors) => {
    if (errors) {
      ...
      model.kind = "Order";
      purchase.addPurchaseOrder(model);
...
<template>
  <container title="采购开单">
    <n-form
      ...
      :rules="rule1"
    >
      <n-form-item label="采购编号:" path="id">
    ...
    >
      <n-space>
        ...
        <n-form-item label="实付金额" path="actal_payment">
          <NInputNumber
    ...
    <n-button type="primary" @click="submitForm"> 创建订单 </n-button>
  </container>
</template>
<style scoped>
.container {
