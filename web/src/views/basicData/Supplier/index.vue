...
type SupplierModel = Partial<Supplier>;
const basic = useBasic();
const message = useMessage();

const showModal = ref(false);
const formRef = ref<FormInst | null>(null);
const model = ref<Partial<SupplierModel>>({});
provide("form", {
  model,
  formRef,
});
...
const handler = () => {
  formRef.value?.validate((errors) => {
    if (!errors) {
      basic.createSupplier(model.value);
...
const eModel = ref<SupplierModel>({});
provide("eform", {
  model: eModel,
  formRef: eFormRef,
});
const editHandler = () => {
  eFormRef.value?.validate((errors) => {
    if (!errors) {
      basic.updateSupplier(eModel.value);
...
const columns: DataTableColumns<SupplierColumns> = [
  {
    title: "序号",
    key: "index",
    render(row, index) {
      ...
      return h("div", {}, [
        h(
          NButton,
          {
            ...
            // text: "编辑",
            onClick: () => {
              const index = basic.supplier.findIndex(
                (item) => item.id == row.id
              );
              const item = basic.supplier[index];
              eModel.value = item;
