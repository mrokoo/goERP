...
type UnitModel = Partial<Unit>;
const goods = useGoods();
const message = useMessage();

const showModal = ref(false);
const formRef = ref<FormInst | null>(null);
const model = ref<Partial<UnitModel>>({});
provide("form", {
  model,
  formRef,
});
...
const columns: DataTableColumns<UnitColumns> = [
  {
    title: "序号",
    key: "index",
    render(row, index) {
      ...
      return h("div", {}, [
        h(
          NButton,
          {
            ...
            // text: "编辑",
            onClick: () => {
              const index = goods.unit.findIndex(
                (item) => item.id == row.id
              );
              const item = goods.unit[index];
              eModel.value = item;
